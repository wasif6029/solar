...
z = [180, 170, 160, 150, 140, 130, 120, 110, 100, 90, 80, 70, 60, 50, 40, 30, 20]


# def newAssumptionOfZ(value_area):
#     res = 0
#     for i in range(len(area)):
#         if i == 0:
#             if value_area < area[i]:
#                 res = 180 - int((420 - value_area) * (10 / (550 - 420)))
#                 break
#             else:
#                 # z[i] + ((value_area - area[i])/(area[i+1]-area[i])*z[i])
#                 return (int((z[i] + z[i + 1]) / 2))
#     if res < 0:
#         res = 0
#     return res
...
def assumptionZ(value_area):
    for i in range(len(area)):
        if i == len(area) - 1:
            return int(area[i] / value_area * z[i])
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = None if hide_labels else (
                            names[c] if hide_conf else f'{names[c]} {math.floor((x1 + x2) / 2), math.floor((y1 + y2) / 2)} {conf:.2f} {assumptionZ(area)}')
